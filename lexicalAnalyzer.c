...
void lexicalAnalyzer(FILE *file)
{
    ...
    printf("Cooke Analyzer :: 11853193\n");

    while ((ch = fgetc(file)) != EOF)
    {
        if (isspace(ch)) // Ignore whitespace and process current lexeme
        {
            if (lexemeIndex > 0)
            {
                ...
                lexemeIndex = 0; // Reset for the next lexeme
            }
        }
        else if (strchr("=+-*/%<>&|!();{}", ch) != NULL) // Handle single-char tokens
        {
            if (lexemeIndex > 0)
            {
                lexeme[lexemeIndex] = '\0';
            ...
            // Handle the single character token
            lexeme[0] = ch;
            lexeme[1] = '\0';
            TokenType token = classifyLexeme(lexeme);
            printf("%s %s\n", lexeme, tokenNames[token]);
